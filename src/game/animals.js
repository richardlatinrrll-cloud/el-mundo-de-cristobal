...
export function makeMesh(def) {
  ...
  // igual que box(), pero redondeada (cápsula) — para patas, menos "de bloque"
  const boxR = (w, h, d, col, x, y, z, rot) => {
    const m = capsula(w * s, h * s, d * s, col);
    m.position.set(x * s, y * s, z * s);
    if (rot) m.rotation.set(rot[0] || 0, rot[1] || 0, rot[2] || 0);
    g.add(m); mats.push(m.material); return m;
  };
  ...
  const patas = (bw, y, cols) => {
    for (const [px, pz] of [[-0.2, 0.3], [0.2, 0.3], [-0.2, -0.3], [0.2, -0.3]]) {
      boxR(bw, y * 2, bw, cols || cD, px, y, pz);
    }
  };
  ...
  if (f === 'ave') {
    ...
    for (const [px, pz] of [[-0.32, 0.32], [0.32, 0.32], [-0.32, -0.32], [0.32, -0.32]]) boxR(0.16, 0.16, 0.2, c, px, 0.14, pz);
    ...
    boxR(0.18, 0.55, 0.18, cD, -0.4, 1.55, 0.5); boxR(0.18, 0.55, 0.18, cD, 0.4, 1.55, 0.5); // bracitos
    boxR(0.42, 1.3, 0.42, cD, -0.42, 0.65, -0.1); boxR(0.42, 1.3, 0.42, cD, 0.42, 0.65, -0.1); // patotas
    ...
    for (const [px, pz] of [[-0.42, 0.5], [0.42, 0.5], [-0.42, -0.5], [0.42, -0.5]]) boxR(0.36, 1.0, 0.36, cD, px, 0.5, pz);
    ...
    boxR(0.12, 0.3, 0.12, cD, -0.22, 0.85, 0.35); boxR(0.12, 0.3, 0.12, cD, 0.22, 0.85, 0.35); // brazos
    boxR(0.2, 0.85, 0.2, cD, -0.24, 0.42, -0.1); boxR(0.2, 0.85, 0.2, cD, 0.24, 0.42, -0.1);   // patas
